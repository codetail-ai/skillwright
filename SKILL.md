...
---
# /skillwright — Level 5 Skill Dark Factory
...
User invokes `/skillwright` followed by their input:

```
/skillwright Every week I pull sales data, clean it, and generate a report
/skillwright https://wiki.internal/deploy-runbook
/skillwright See scripts/invoice_processor.py — turn it into a reusable skill
/skillwright Here's our API docs: https://api.internal/docs — make a skill for querying inventory
/skillwright Based on compliance-checklist.pdf, create a skill for SOX audits
```
...
**Output**: A self-contained skill that is installed and invoked the same way as skillwright itself:
...
Once installed, anyone on any platform types `/skill-name` and the skill activates — exactly like `/skillwright` or `/clarity`. The generated skill is a first-class citizen, not a second-class output.
...
4. **Optionally publish to the team registry** (if the skillwright registry is available):
   ```bash
...
1. The consultant teaches each team member to install and use skillwright
2. The consultant creates one shared `{team}-skills-registry` repo on GitHub/GitLab
3. Each team member creates skills from their own workflows using `/skillwright`
...
5. **Give the user a team onboarding guide** they can share on Slack, Teams, or email:
   ...
   STEP 3: Create a skill from any workflow you do repeatedly
     ...
     Open your IDE chat and type:
       /skillwright Every week I pull sales data and generate a report
...
"A newer version of skillwright is available. Run `git pull` in <path> to update."
